...
int main()
{
    ...
    cout << "Octal Number:" << calculationFinal(hexadecimalNumber) << endl;
    system("pause");
    return 0;
}
bool isHexadecimal(string str)
...
string calculationFinal(string str)
{
    ...
    string beforePoint{NULL}, afterPoint{NULL}, octalNumber{NULL};
    if (decimalPoint(decimalNumber) != decimalNumber.length())
    {
        ...
        {
            int n = numberBeforePoint % 8;
            octalNumber = to_string(n) + octalNumber;
            numberBeforePoint /= 8;
        }
        octalNumber += '.';
        while (numberAfterPoint > 0 && octalNumber.length() < 32)
        {
            numberAfterPoint *= 8;
            int n = numberAfterPoint;
            octalNumber += to_string(n);
            numberAfterPoint -= n;
        }
    }
    else
